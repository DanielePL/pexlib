...
function Login() {
  ...
  const { login } = useAuth(); // Verwende den AuthContext
  ...
  const handleSubmit = async (e) => {
    ...
    if (password1 === validPassword1 && password2 === validPassword2) {
      ...
      try {
        // Option 1: API-basiertes Login (falls Backend vorhanden)
        ...
        console.warn('⚠️ API login failed, using fallback:', apiError);

        // Fallback: Lokale Authentifizierung
        const token = 'team-authenticated-' + Date.now();

        // Speichere in localStorage
        localStorage.setItem('authToken', token);

        // Verwende AuthContext
        login(token, { id: 'admin', role: 'admin' });
      ...
      setError('Invalid password combination. Please try again.');
      setIsLoading(false);
    }
  };
      ...
      <div style={{
        ...
        {process.env.NODE_ENV === 'development' && (
          <div style={{
            ...
            Debug: Expected passwords are "Kraft" and "Vision"
          </div>
        )}
